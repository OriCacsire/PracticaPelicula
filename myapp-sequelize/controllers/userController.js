...
const userController = {
    ...
    let rememberme =req.body.rememberme
        ...
        //VALIDO LA RECUPERACION DE LOS DATOS--> FUNDAMENTAL

    let errors = {}; // en este se van a ir cuardando los errores. Se hace un condicionar para si el emailBuscado es vacio, entonces se muestra un error
    if (emailBuscado == ""){ 
        errors.message = "El campo email esta vacio"; //se esta incorporando un mensaje
        res.locals.errors = errors;//guardo en locals osea en el servidor para enviar a las vistas el objeto literal errors
        return res.render("login");

        //como login no recibe parametros, pero si recibe parametros seria un redirect
    } else if(req.body.password == ""){    //Se recupera aca, pero tmb se puede guardar en una variable y luego usarlo.
        errors.message = "El campo contraseña esta vacio"; 
        res.locals.errors = errors;
        return res.render("login");

    }else{
        /*si todos los campos cumple su condicion, se ejecuta esto. Lo que se armo antes. */
        let criterio = {
        ...
        .then ((result) =>{ //Manejamos el resultado de la búsqueda del usuario en una promesa, cuando se ingrese algo por el buscador recien se ejecuta esto
            if (result != null){//Si se encuentra un usuario, es decir si el usuario no es nulo se procede a verificar la contraseña, viendo si la contraseña que se esta pasando es igual a la que tenemos en la base de datos --> utilizando bcrypt.compareSync. 
                ...
                if (check) {
                    ...
                    //aca el usuario se crea en session
